...
(function () {
  ...
  var BASE_IDX = (MOBILE || SAVEDATA) ? 0 : 2;
  if (DEBUG) {
    var m = location.search.match(/[?&]gl2n=(\d+)/);
    if (m) {
      var dn = Math.max(16, Math.min(1024, parseInt(m[1], 10)));
      SIZES = [Math.max(16, dn >> 1), dn, Math.min(1024, dn << 1)];
      BASE_IDX = 1;
    }
  }
  var N = SIZES[BASE_IDX]; // reassigned by governor resizes (managed reinit)
  ...
  var COMP_FS = [
    ...
    "  vec2 c = vUv - 0.5;",
    "  float r = length(c);",
    "  float k = 0.0045 * r * uPostMix * uAberr;",
    "  vec3 field;",
  ...
  var COUNT = N * N; // reassigned alongside N on governor resizes
  ...
  function allocPostTargets() {
    // governor axis 2 (fill rate): rung ≥ 1 shrinks bloom to eighth-res,
    // rung ≥ 2 runs the whole post chain at half resolution
    var gl = state.gl, w = state.canvas.width, h = state.canvas.height;
    var ts = state.gov && state.gov.rung >= 2 ? 0.5 : 1;
    var gshift = state.gov && state.gov.rung >= 1 ? 3 : 2;
    ["trailA", "trailB", "glowA", "glowB"].forEach(function (k) { destroyFBO(gl, state[k]); state[k] = null; });
    var tw = Math.max(2, Math.round(w * ts)), th = Math.max(2, Math.round(h * ts));
    state.trailA = makeFBO(gl, tw, th);
    state.trailB = makeFBO(gl, tw, th);
    var qw = Math.max(2, tw >> gshift), qh = Math.max(2, th >> gshift);
  ...
  function buildGLResources() {
    ...
    if (!MOBILE && !SAVEDATA) {
      try {
        ...
        state.progComp = { p: pc, aXY: gl.getAttribLocation(pc, "aXY"), uTex: gl.getUniformLocation(pc, "uField"), uGlow: gl.getUniformLocation(pc, "uGlow"), uPostMix: gl.getUniformLocation(pc, "uPostMix"), uAberr: gl.getUniformLocation(pc, "uAberr") };
  ...
  function effDprCap() {
    // governor axis 2, rung 3: cap the backing store at 1.0 device px
    return state.gov && state.gov.rung >= 3 ? 1.0 : DPR_CAP;
  }
  function resize() {
    var c = state.canvas, dpr = Math.min(window.devicePixelRatio || 1, effDprCap());
  ...
  function setMorphPair(a, b, t) {
    ...
    if (Math.abs(state.mix - prev) > 0.002) {
      excite(Math.min(1, 0.4 + Math.abs(state.mix - prev) * 30));
      rmWake(); // an actual scrub movement is a command
    }
  }
  ...
  function frame(now) {
    ...
    var gl = state.gl;
    now *= 0.001;
    var dtMs = (now - state.lastT) * 1000; // real presented interval (governor metric)
    ...
    function drawPoints() {
      ...
      gl.uniform1f(state.renLoc.uSize, (MOBILE ? 1.35 : 1.6) * Math.min(window.devicePixelRatio || 1, effDprCap()) * (state.post && state.trailB ? state.trailB.h / state.canvas.height : 1));
    ...
    if (state.post) {
      ...
      gl.viewport(0, 0, state.trailB.w, state.trailB.h); // governed size, not canvas size
      ...
      drawQuad(state.progComp, state.trailB.tex, function () {
        ...
        gl.uniform1f(state.progComp.uAberr, state.gov && state.gov.rung >= 1 ? 0 : 1);
        gl.activeTexture(gl.TEXTURE0);
      });
  ...
  /* ═══ governor — two axes: post/fill quality (rungs 0–5) and sim size ═══
     Metric: p90 frame time per window (5s; 1s during the startup fast
     path). Good < 17.9ms, bad > 25ms, emergency = rolling 1s mean > 50ms.
     Degrade order: aberration+bloom → post half-res → DPR cap → sim size
     down (idle-deferred reinit) → post off → tier demotion. Improvement
     reverses; promotion above baseline is desktop-only and gated on a
     trial allocation (no retry per session on failure). */
  var govPersist = null, seedPersist = null;
  function initGov() {
    state.gov = {
      // debug pages are test instruments: live collection would let
      // SwiftShader's real frame times fight the injector (emergencies,
      // even mid-test resizes tearing down engines under inspection).
  ...
  function govTrialAlloc(n) {
    ...
    try {
      ...
      var ok = gl.checkFramebufferStatus(gl.FRAMEBUFFER) === gl.FRAMEBUFFER_COMPLETE && gl.getError() === gl.NO_ERROR;
      gl.bindFramebuffer(gl.FRAMEBUFFER, null);
      gl.deleteTexture(t);
  ...
  function govDegradeOnce() {
    ...
    if (g.rung < 3) {
      // two-strike rung-promotion memory (real-hardware finding: a
      // machine whose rung-N p90 sits just under the good line and whose
      // rung-(N-1) p90 sits in the hold band flickers post effects every
      ...
      if (promo && promo.to === g.rung && state.time - promo.t < 120) {
        ...
        strikes.n++;
        strikes.t = state.time;
        g.rungStrikes = strikes;
        if (strikes.n >= 2) {
          g.rungLockAt = promo.to + 1;
          govLog("rung-lock", { at: g.rungLockAt });
          if (window.console) console.info("[DMDS] governor: rung " + promo.to + " promotion failed twice — holding rung " + g.rungLockAt + " (re-armed on tab revisit)");
        }
        g.lastRungPromo = null;
      }
      applyRung(g.rung + 1);
    }
    else if (g.pendingResize) {
      // sustained bad must not starve behind idle-deferral (real-hardware
      // finding: a continuously-interacted machine demoted from 512²
      // having never actually run a smaller size — the deferred downsizes
      // starved and the demote verdict cited evidence that was never
      // collected). The deferral protects a smooth field from a visible
      // pop; a field missing its frame budget is not smooth. Force the
      // queued downsize at the next frame boundary — further descent uses
      // evidence collected AT the smaller size, never at the size the
      // ladder already left.
  ...
  function govImproveOnce() {
    ...
    if (g.rung === 5) applyRung(3);
      ...
      // rungs restore BEFORE size promotions: a size promotion may only
      // execute from rung 0 (govMaybeResize), so requesting one while
      // rungs are degraded created a request/cancel loop that starved
      // rung recovery entirely once a downsize had actually applied
      if (g.rung - 1 < (g.rungLockAt || 0)) return; // locked pair: hold here
  ...
  function govFrame(dtMs, now) {
    if (REDUCED) return; // reduced motion: static field, no governing
    var g = state.gov;
    if (g.demoted || g.liveOff) return; // liveOff: a test injector owns the governor
    if (!g.warmed) {
      // warm-up: compile/upload stalls never count against the page
      if (!g.warmT0) g.warmT0 = now;
      g.warmFrames++;
      if (g.warmFrames >= 30 || now - g.warmT0 >= 1) {
        g.warmed = true;
        g.fastUntil = now + 3;
        g.winStart = now;
        g.frames = [];
  ...
  function executeResize(idx) {
    ...
    return attempt(idx).then(function () {
      ...
      // a size change moves the perf landscape: rung-bounce evidence
      // collected at the old size doesn't transfer — fresh chance
      g.rungLockAt = 0;
      g.rungStrikes = null;
      g.lastRungPromo = null;
  ...
  function init(canvas, onMilestone) {
    ...
    // governor survives a resize reinit (it ordered it); fresh boots start clean
    if (govPersist) { state.gov = govPersist; govPersist = null; }
    else initGov();
    state.rmSleeping = false;
    state.lastCommandT = performance.now() * 0.001;
    ...
    listen(canvas, "webglcontextrestored", function () {
      ...
      try {
        ...
        state.mix = 1;
        state.mode = "tween";
        if (state.gov) { state.gov.invalid = true; state.gov.cool = state.time + 5; }
    ...
    return fontsReady.then(function () {
      ...
      if (seedPersist) {
        // governor resize: re-seed AT the surviving formation's targets
        // (small jitter) so the field re-assembles in well under a second
        var keepName = seedPersist;
        seedPersist = null;
        var keep = formationFor(keepName) || state.formations.logo;
        uploadTarget(gl, state.targA, keep, dustOf(keepName));
        uploadTarget(gl, state.targB, keep, dustOf(keepName));
        state.currentName = keepName;
        var reseed = new Float32Array(COUNT * 4);
        for (var ri = 0; ri < COUNT; ri++) {
          reseed[ri * 4] = keep[ri * 3] + gauss() * 1.5;
          reseed[ri * 4 + 1] = keep[ri * 3 + 1] + gauss() * 1.5;
          reseed[ri * 4 + 2] = keep[ri * 3 + 2] + gauss() * 1.5;
          reseed[ri * 4 + 3] = DEPTH_FREE;
        }
        gl.bindTexture(gl.TEXTURE_2D, state.posT[state.cur]);
        gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, N, N, gl.RGBA, gl.FLOAT, reseed);
        state.mix = 1;
        state.mode = "tween";
        state.tweenDur = state.morphDur = REDUCED ? 0.01 : 1.5;
        state.excite = 0.6;
      } else {
  ...
  window.DMDS_GL2 = {
    ...
    status: function () {
      ...
      return {
        ...
        excite: Math.round(state.excite * 100) / 100
      };
    },
    onDemote: function (fn) { state.onDemote = fn; },
    ...
    debugGovInject: function (frameMs, opts) {
      ...
      // frame loop uses — tests exercise real decision logic
      if (!DEBUG) throw new Error("debugGovInject requires ?debug=1");
      opts = opts || {};
      var g = state.gov;
      g.liveOff = true; // the injector owns the governor from first use —
